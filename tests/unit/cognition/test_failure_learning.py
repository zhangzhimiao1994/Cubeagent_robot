...
def test_cognitive_failure_observation_replaces_unsafe_tokens() -> None:
    ...
    assert payload["tags"] == ["cognition", "failure", "unknown_stage"]
    assert lesson == (
        "cognition_failure stage=unknown_stage failure_class=unknown_failure "
        "impact=unknown_impact strategy=unknown_strategy"
    )
