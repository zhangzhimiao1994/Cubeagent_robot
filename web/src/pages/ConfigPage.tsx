  ...
  return (
    <section>
      ...
      <div className="settings-shortcuts">
        ...
        <Link className="settings-shortcut-card" to="/voice-models">配置语音模型</Link>
      ...
      <form onSubmit={submitSettings} aria-label="保存系统设置" className="settings-form">
        ...
        <div className="inline-guide" role="region" aria-label="机器人语音配置入口">
          <h3>机器人语音</h3>
          <p>
            当前{settings.robot_voice.enabled ? `已启用 ${settings.robot_voice.media_provider}` : "未启用"}；
            默认音色：{settings.robot_voice.default_voice_id ?? settings.robot_voice.minimax_tts_voice_id ?? "未设置"}。
            API Key、ASR/TTS 模型、音色列表和声音克隆任务统一在独立语音模型页维护。
          </p>
          <Link className="secondary-action" to="/voice-models">打开语音模型配置</Link>
        </div>
